...
Future<String?> fetchStationInfo(String trainCode) async {
  try {
    ...
    logError(from: 'emu_search_page/fetchStationInfo', error: e.toString());
    logError(
...
Future<String?> fetchRouteByMoeFactory(String emuNo) async {
  try {
    ...
    logError(from: 'emu_search_page/fetchRouteByMoeFactory', error: e.toString());
    return null;
  }
}
...
Future<String?> fetchRouteByRailGo(String emuNo) async {
  try {
    ...
    logError(from: 'emu_search_page/fetchRouteByRailGo', error: e.toString());
    return null;
  }
}
...
Future<String?> fetchRouteByRailRe(String emuNo) async {
  try {
    ...
    logError(from: 'emu_search_page/fetchRouteByRailRe', error: e.toString());
    return null;
  }
}
...
class _SearchPageState extends State<SearchPage> {
  ...
  bool showRoutes = false; // 在 initState 中根据平台和搜索类型设置
  ...
  void initState() {
    ...
    // Web 版：车号查询默认关闭交路查询
    // if (kIsWeb) showRoutes = true; // 已移除，保持默认关闭
    _loadConfig();
  }
  ...
  Future<void> _loadConfig() async {
    try {
      ...
      logError(from: 'emu_search_page/_loadConfig', error: e.toString());
      logError(
  ...
  Future<void> _searchByTrainCode(
    ...
    // Web 版直接跳转 rail.re
    if (kIsWeb) {
      launchUrl(Uri.parse('https://rail.re/#$fullCode'));
      return;
    }
    ...
    try {
      ...
      logError(from: 'emu_search_page/_searchByTrainCode', error: e.toString());
      logError(
      ...
      await Future.wait(
        bestRecords.map((record) async {
          ...
          try {
            ...
            logError(from: 'emu_search_page/_searchByTrainId', error: e.toString());
            routeMap[emuNo] = null;
          }
        }),
      );
  ...
  Future<void> _performSearch() async {
    ...
    try {
      ...
      logError(from: 'emu_search_page/_performSearch', error: e.toString());
      logError(
  ...
  String? _checkDateFreshness(List<dynamic> data, AppSettings settings) {
    ...
    for (final item in data) {
      ...
      } catch (e) { logError(from: 'emu_search_page/_dataSourceLabel', error: e.toString()); }
    }
  ...
  Widget _buildResultCard(SearchResult result) {
    ...
    return InkWell(
      onTap: () {
        if (kIsWeb) {
          // Web 版：打开 rail.re 对应页面
          final code = result.trainCodeForJourney ?? result.number;
          if (code.isNotEmpty) {
            launchUrl(Uri.parse('https://rail.re/#$code'));
          }
        } else if (canNavigateToJourney) {
  ...
  Widget _buildErrorSection(AppSettings settings) {
    return Column(
      children: [
        ...
        const SizedBox(height: 12),
        if (searchType == 'trainCode' && !kIsWeb)
          IntrinsicHeight(
            child: Row(
              children: [
                Expanded(
