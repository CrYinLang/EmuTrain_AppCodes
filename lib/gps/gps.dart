...
class TrackRecord {
  ...
  static Future<bool> save(TrackRecord record) async {
    if (kIsWeb) return false; // Web 不支持文件存储
  ...
  static Future<List<TrackRecord>> loadAllMeta() async {
    ...
    for (final dir in entries) {
      ...
      } catch (e) { logError(from: 'gps/save', error: e.toString()); }
    }
    return records;
  }

  static Future<TrackRecord?> loadFull(String id) async {
    if (kIsWeb) return null; // Web 不支持文件存储
    ...
    try {
      ...
      logError(from: 'gps/loadFull', error: e.toString());
      return null;
    }
  }
...
class _TrackDetailPageState extends State<TrackDetailPage> {
  ...
  Future<void> _loadFullRecord() async {
    try {
      ...
      logError(from: 'gps/_loadFullRecord', error: e.toString());
      if (mounted) {
        setState(() {
          _isLoading = false;
          _error = '加载失败';
