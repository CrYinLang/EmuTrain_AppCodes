...
class __JourneyDetailContentState extends State<_JourneyDetailContent>
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        journey.stations.any((s) => s.stationName.isEmpty)
                            ? '包含自定义车站，不支持工具箱'
                            : '自定义旅途，仅支持本地线路图',
                        style: TextStyle(
                            fontSize: 11,
  ...
  Widget _buildLineMapButton(BuildContext context) {
    ...
    // 自定义旅途（纯国铁站）或普通旅途
    return ElevatedButton.icon(
      onPressed: () => _showLineMapToolbox(context),
      icon: const Icon(Icons.map),
      label: const Text('线路走向图'),
    );
  }

  // 简单判断：国铁车站名通常只有2-4汉字（不含特殊标记）
  bool _isRailStation(String name) {
    return RegExp(r'^[一-龥]{2,6}$').hasMatch(name);
  }
