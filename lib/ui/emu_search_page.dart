...
class _SearchPageState extends State<SearchPage> {
 ...
 logError(from: 'EmuSearchPage._getStationInfo', error: e.toString(), level: 2);
 return null;
 }
 }
 ...
 () {
 logError(from: 'EmuSearchPage', error: 'RAILGO数据源已停用，用户需切换数据源', level: 3);
 ...
 logError(from: 'EmuSearchPage._performSearch', error: '车次不存在: $fullCode', level: 3);
 setState(
 () => errorMsg =
 '车次不存在!\n当前数据源: ${settings.dataSource.toString().split('.').last}',
 );
 ...
 logError(from: 'EmuSearchPage._performSearch[parse]', error: e.toString(), level: 4);
 setState(
 () => errorMsg =
 '解析返回数据失败: $e\n当前数据源: ${settings.dataSource.toString().split('.').last}',
 );
 ...
 logError(from: 'EmuSearchPage._performSearch', error: e.toString(), level: 4);
 ...
 if (searchType == 'bureau' || searchType == 'carType' || searchType == 'depot')
 Text(
...
Widget _buildMoreMenu() {
 ...
 final int totalCount = (searchType == 'bureau' || searchType == 'carType' || searchType == 'depot')
 ? _totalResults
 : displayedCount;
 ...
 if (value == 'coach') {
 Navigator.of(context).push(
 MaterialPageRoute(builder: (context) => const CoachSearchPage()),
 );
 } else if (value == 'loco') {
 Navigator.of(context).push(
 MaterialPageRoute(builder: (context) => const LocoSearchPage()),
 );
 }
 },
 itemBuilder: (context) => [
...
const SizedBox(height: 20),
 ...
 buildResultCountBar(
 context,
 label: (searchType == 'bureau' || searchType == 'carType' || searchType == 'depot')
