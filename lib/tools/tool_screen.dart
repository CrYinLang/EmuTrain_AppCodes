...
import '../gps/gps.dart';
import '../route/route_hub_page.dart';
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            ...
            Card(
              color: kIsWeb ? Theme.of(context).colorScheme.surfaceContainerHighest.withValues(alpha: 0.5) : null,
              child: ListTile(
                leading: Icon(Icons.av_timer, size: 32, color: kIsWeb ? Theme.of(context).hintColor : null),
                title: Text('速度计', style: TextStyle(color: kIsWeb ? Theme.of(context).hintColor : null)),
                subtitle: Text(kIsWeb ? '网页端不可用' : '实验性功能，可能不准', style: TextStyle(color: kIsWeb ? Theme.of(context).hintColor : null)),
                trailing: kIsWeb ? null : const Icon(Icons.arrow_forward_ios),
                onTap: kIsWeb ? null : () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
